...
SELECT  
 ...
 Job.ID AS JobID,  
 CASE  
  WHEN Stats.ID = 1 THEN Job.Number  
 ...
 ON Job.ID = Inf.JobID LEFT OUTER JOIN CS_Job_JobStatus JobStats (NOLOCK)
 ON Inf.JobID = JobStats.JobID AND JobStats.Active = 1 LEFT OUTER JOIN CS_JobStatus Stats (NOLOCK)  
 ON JobStats.JobStatusID = Stats.ID LEFT OUTER JOIN  
