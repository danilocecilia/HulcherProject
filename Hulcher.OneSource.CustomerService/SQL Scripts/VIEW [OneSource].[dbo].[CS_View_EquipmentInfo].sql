DROP VIEW CS_View_EquipmentInfo

GO

--USED FOR THE PRIMARY LISTING OF EQUIPMENTS COMBO
--AND SOLO EQUIPMENTS
CREATE VIEW CS_View_EquipmentInfo
AS
  
SELECT
 CASE
  WHEN EqCombo.ID IS NULL THEN Eq.ID
  ELSE 0
 END AS EquipmentID,
 EqCombo.Name AS ComboName,
 CASE
  WHEN EqCombo.ID IS NULL THEN CONVERT(BIT, 0)
  ELSE CONVERT(BIT, 1)
 END AS IsCombo,
 ...
 ON Eq.ID = EqCombo.PrimaryEquipmentID INNER JOIN CS_Division Div (NOLOCK)
 ...
 WHERE Eq.ID = EqCombo.PrimaryEquipmentID
 OR Eq.ComboID IS NULL