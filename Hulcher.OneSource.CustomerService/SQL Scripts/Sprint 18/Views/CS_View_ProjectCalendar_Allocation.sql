...
/****** Object:  View [dbo].[CS_View_ProjectCalendar_Allocation]    Script Date: 01/03/2012 17:50:30 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[CS_View_ProjectCalendar_Allocation]    
AS    
...
SELECT DISTINCT    
 ...
 , DATEADD(d, res.Duration - 1, res.StartDateTime) AS [ResourceEndDateTime]      
...
FROM    
 ...
 LEFT OUTER JOIN CS_Resource res (NOLOCK) ON job.ID = res.JobID
 LEFT OUTER JOIN CS_Employee emp (NOLOCK) ON res.EmployeeID = emp.ID    
 LEFT OUTER JOIN CS_Equipment equip (NOLOCK) ON res.EquipmentID = equip.ID    
