ALTER VIEW [dbo].[CS_View_GetJobData] 
AS
...
SELECT
	...
	, primarycontact.Alias AS [PrimaryContactAlias]
	...
	, pocemployeediv.ID		AS [POCEmployeeDivisionID]
	, pocemployeediv.Name		AS [POCEmployeeDivisionName]
	, cinfodiv.ID as [DivisionID]
	, cinfodiv.Name as [DivisionName]
	, pdiv.ID		AS [PrimaryDivisionID]
	, pdiv.Name		AS [PrimaryDivisionName]
	...
	, onsitecontact.Alias AS [OnSiteContactAlias]
	...
	, secondarycontact.Alias AS [SecondaryContactAlias]
	...
	, billtocontact.Alias AS [BillToContactAlias]
...
FROM
	CS_Job job (NOLOCK)
		INNER JOIN CS_CustomerInfo cinfo (NOLOCK) on cinfo.JobId = job.ID
			...
			LEFT OUTER JOIN CS_Division pocemployeediv (NOLOCK) on pocemployeediv.ID = pocemployee.DivisionID
			...
			INNER JOIN CS_JobInfo jinfo (NOLOCK) on jinfo.JobID = job.ID
				INNER JOIN CS_Job_JobStatus jstatus (NOLOCK) on jstatus.JobID = jinfo.JobID and jstatus.Active = 1
					INNER JOIN CS_JobStatus jstat (NOLOCK) on jstatus.JobStatusId = jstat.ID
			...
			INNER JOIN CS_JobDivision jdiv (NOLOCK) on jdiv.JobID = job.ID
			INNER JOIN CS_Division pdiv (NOLOCK) on jdiv.DivisionID = pdiv.ID
