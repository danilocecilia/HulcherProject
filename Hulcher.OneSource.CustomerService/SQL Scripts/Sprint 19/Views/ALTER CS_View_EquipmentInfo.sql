ALTER VIEW CS_View_EquipmentInfo        
AS              
...
SELECT DISTINCT            
  ...
  CAST(
  CASE
   WHEN whitelight.EquipmentID IS NULL THEN 0
   ELSE 1
   END AS BIT) AS [IsWhiteLight]
 ...
 FROM            
  ...
  LEFT OUTER JOIN CS_EquipmentWhiteLight whitelight (NOLOCK) ON equip.ID = whitelight.EquipmentID     
  ...
  LEFT OUTER JOIN CS_CallLogResource CRes (NOLOCK) ON TRes.ID = CRes.ID AND CRes.Active = 1            
  ...
  LEFT OUTER JOIN CS_View_ConflictedEquipmentCombos divConf (NOLOCK) ON combo.ID = divConf.ComboID          
  LEFT OUTER JOIN           
  (          
 SELECT          
  perm.EquipmentId AS EquipmentID           
 FROM     CS_EquipmentPermit perm (NOLOCK)          
 WHERE           
