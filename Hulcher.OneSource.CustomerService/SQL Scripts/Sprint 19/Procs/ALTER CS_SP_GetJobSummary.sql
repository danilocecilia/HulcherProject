...
SELECT DISTINCT           
 ...
 ,j.ID             AS [JobStatusId]            
 ,st.Acronym + ', ' + st.Name + ', ' + CASE cty.CSRecord WHEN 1 THEN '* ' + cty.Name ELSE cty.Name END  AS [Location]            
 ...
 ,CONVERT(bit,             
 CASE             
 WHEN COUNT(rs.ID) > 0 OR COUNT(rsv.ID) > 0 THEN 1             
 ELSE 0             
 END)            AS [HasResources]            
...
FROM            
 ...
 LEFT OUTER JOIN cs_Reserve rsv (NOLOCK) ON g.ID = rsv.JobId AND rsv.Active = 1          
 ...
 LEFT JOIN CS_Employee resvEmp (NOLOCK) ON rsv.EmployeeID = resvEmp.ID
...
WHERE  
 ...
 AND (CASE WHEN @dateFilterType = 1 THEN a.InitialCallDate  
  ...
  AND (((resEmp.Name + ', ' + resEmp.FirstName like '%' + @PersonName + '%')
     OR (resvEmp.Name + ', ' + resvEmp.FirstName like '%' + @PersonName + '%')
...
GROUP BY  
 ...
 ,j.Description            
 ,j.ID            
 ,st.Acronym + ', ' + st.Name + ', ' + CASE cty.CSRecord WHEN 1 THEN '* ' + cty.Name ELSE cty.Name END            
 ,a.ModificationDate            
 ,cl.ID            
 ,ct.Description            
 ,cl.CallDate            
...
SELECT DISTINCT              
 ...
 ,i.Name + ' - ' + i.Description           AS [EquipmentName]         
 ,NULL             AS [ProjectManager]            
 ,j.Description           AS [JobStatus]            
 ,j.ID             AS [JobStatusId]            
 --,st.Acronym + ', ' + st.Name + ', ' + cty.Name   AS [Location]            
 ,divstate.Acronym + ', ' + divstate.Name AS [Location]
...
FROM            
 ...
 LEFT OUTER JOIN CS_State divstate (NOLOCK) ON div.StateID = divstate.ID
...
GROUP BY
 ...
 ,divstate.Acronym + ', ' + divstate.Name
 ,g.ModificationDate
 ,d.ModificationDate
 ,cl.ID
 ,ct.Description
...
SELECT DISTINCT              
 ...
 ,CONVERT(bit, 1)          AS [IsResource]            
 ,(x.Name + ', ' + x.FirstName)       AS [EmployeeName]            
 ,i.Name + ' (' + CAST(dcount.[Count] AS VARCHAR(50)) + ')'          AS [EquipmentType]            
 ,NULL             AS [ProjectManager]            
 ,j.Description           AS [JobStatus]            
 ,j.ID             AS [JobStatusId]            
 --,st.Acronym + ', ' + st.Name + ', ' + cty.Name   AS [Location]            
...
FROM            
 ...
 ON a.ID = dcount.JobID AND d.JobID = dcount.JobID AND d.EquipmentTypeID = dcount.EquipmentTypeID AND d.DivisionID = dcount.DivisionID
 LEFT OUTER JOIN CS_Employee resmod (NOLOCK) ON resmod.ID = d.ModificationID            
 LEFT OUTER JOIN CS_Employee x (NOLOCK) ON d.EmployeeID = x.ID            
