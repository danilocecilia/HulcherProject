...
/****** Object:  View [dbo].[CS_View_EquipmentInfo]    Script Date: 07/26/2011 18:45:12 ******/
SET ANSI_NULLS ON
GO
...
AS    
 ...
 SELECT DISTINCT  
  ...
  ,CAST(
	CASE 
		WHEN (perm.Id IS NOT NULL AND pmail.ID IS NULL)
		THEN 1 
		ELSE 0 
	END   
 ...
 FROM  
  ...
  LEFT OUTER JOIN CS_EquipmentPermit perm ON (equip.ID = perm.EquipmentId AND ExpirationDate <= GETDATE())
  LEFT OUTER JOIN CS_EquipmentPermitEmail pmail ON perm.ID = pmail.EquipmentPermitID
